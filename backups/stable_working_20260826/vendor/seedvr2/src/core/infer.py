        ...
        if len(samples) > 0:
            ...
            for sample in batches:
                if os.environ.get("SEEDVR2_TRT_ENCODER", "0") == "1":
                    if os.environ.get("SEEDVR2_TRT_ENCODER_LEGACY", "0") == "1":
                        from .trt_encoder_legacy import encode as trt_encode
                    elif os.environ.get("SEEDVR2_TRT_ENCODER_FAST", "0") == "1":
                        from .trt_encoder_fast import encode as trt_encode
                    else:
                        from .trt_encoder import encode as trt_encode
                    latent = trt_encode(sample)
