...
def _capture_pending_vae_latents(runner, ctx: Dict[str, Any], debug: Debug) -> None:
    """Capture tensors and metadata when stopping immediately before VAE."""
    capture_dir = os.environ.get("SEEDVR2_LATENT_CAPTURE_DIR")
    if not capture_dir:
        return
    ...
    for index, source in enumerate(ctx.get("all_upscaled_latents", []), start=1):
        if source is None:
            continue
        latent = source.detach()
        # Match VideoDiffusionInfer.vae_decode's preprocessing exactly.
        batch = latent.unsqueeze(0)
        batch = batch / scale + shift
        batch = optimized_channels_to_second(batch).squeeze(2)
        payload = {
            "latent": batch.cpu(),
            "source_latent": latent.cpu(),
            "shape": tuple(batch.shape),
            "dtype": str(batch.dtype),
            "batch_index": index,
            # The final batch may be uniformly padded to 4n+1 frames. Keep the
            # real frame count so downstream decoders can discard only the
            # synthetic tail frames after temporal VAE decoding.
            "original_frames": (ctx.get("all_ori_lengths", [None] * index)[index - 1]
                                 if index - 1 < len(ctx.get("all_ori_lengths", [])) else None),
        }
        output = path / f"vae_latent_{index:03d}.pt"
        torch.save(payload, output)
        debug.log(f"Captured VAE latent: {output} shape={tuple(batch.shape)}",
                  category="vae", indent_level=1, force=True)
       ...
       # Skip initial frames
        ...
        # Single GPU: stream in main process
            ...
            for result in _stream_video_chunks(
                ...
                if args.stop_before_vae:
                    # The DiT phase has already saved the exact VAE inputs.
                    # Do not create an empty video or enter post-processing.
                    cap.release()
                    debug.log("Stop-before-VAE test complete; no output video written.",
...
def _process_frames_core(
    ...
    if args.stop_before_vae:
        _capture_pending_vae_latents(runner, ctx, debug)
        debug.log("Stop-before-VAE test mode: DiT complete; latent capture finished. Skipping VAE decode.",
                  category="vae", force=True)
        return torch.empty((0, 0, 0, 0), dtype=torch.float32)
...
"""
    ...
    debug_group.add_argument("--stop_before_vae", action="store_true",
                        help="Capture upscaled VAE inputs and stop after DiT upscaling; test mode only")
