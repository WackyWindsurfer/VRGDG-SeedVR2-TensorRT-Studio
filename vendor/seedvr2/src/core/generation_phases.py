...
def _capture_vae_latent(latent: torch.Tensor, capture_dir: Optional[str], batch_idx: int,
                        debug: Optional['Debug'] = None) -> None:
    """Optionally persist an exact VAE input for repeatable backend tests."""
    if not capture_dir:
        return
    path = os.path.abspath(os.path.expanduser(capture_dir))
    os.makedirs(path, exist_ok=True)
    payload = {
        "latent": latent.detach().to(device="cpu"),
        "shape": tuple(latent.shape),
        "dtype": str(latent.dtype),
        "device": str(latent.device),
        "batch_index": batch_idx,
    }
    output = os.path.join(path, f"vae_latent_{batch_idx:03d}.pt")
    torch.save(payload, output)
    if debug is not None:
        debug.log(f"Captured VAE latent: {output} shape={tuple(latent.shape)}",
...
def decode_all_batches(
    ...
    latent_capture_dir = os.environ.get("SEEDVR2_LATENT_CAPTURE_DIR")
    if latent_capture_dir:
        debug.log(f"VAE latent capture enabled: {os.path.abspath(latent_capture_dir)}",
