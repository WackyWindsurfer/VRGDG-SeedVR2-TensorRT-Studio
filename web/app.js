...
const defaultBatchSizes = {'TensorRT VAE (experimental)': [5, 21], 'SeedVR2 AI': [1, 5, 9, 13, 17, 21, 33, 45]};
const updateBatchOptions = (backend, preferred = null) => { const values = (window.__seedvrBatchSizes?.[backend] || defaultBatchSizes[backend] || defaultBatchSizes['SeedVR2 AI']).map(String); const select = $('#batch-size'); const current = preferred == null ? select.value : String(preferred); select.innerHTML = ''; values.forEach((value) => { const option = document.createElement('option'); option.value = value; option.textContent = value; select.append(option); }); select.value = values.includes(current) ? current : (values.includes('21') ? '21' : values[0]); $('#batch-note').textContent = backend === 'TensorRT VAE (experimental)' ? 'TensorRT supports temporal batches 5 and 21.' : 'SeedVR2 AI supports 4n+1 temporal batch sizes.'; };
$('#backend').addEventListener('change', () => updateBatchOptions($('#backend').value));
